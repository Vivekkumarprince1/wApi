...
async function dispatchBotOutbound(opts: {
  ...
  try {
    ...
    const resolveRes = await fetch(`${contactServiceUrl}/internal/v1/contacts/resolve`, {
      ...
      headers: {
        ...
        'x-internal-service-secret': process.env.INTERNAL_SERVICE_SECRET || 'dev-internal-service-secret-change-me',
  ...
  const bspRes = await fetch(`${bspUrl}/internal/v1/bsp/messages/send`, {
    ...
    headers: {
      'Content-Type': 'application/json',
      'x-internal-secret': process.env.INTERNAL_SERVICE_SECRET || 'dev-internal-service-secret-change-me',
