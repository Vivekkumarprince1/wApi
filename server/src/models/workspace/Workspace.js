...
const WorkspaceSchema = new mongoose.Schema({
  ...
  bspPhoneStatus: {
    type: String,
    enum: ['PENDING', 'CONNECTED', 'DISCONNECTED', 'BANNED', 'FLAGGED', 'RATE_LIMITED', 'UNKNOWN'],
    default: 'PENDING'
  },
...
WorkspaceSchema.methods.ensureWorkspaceBspReady = function () {
  ...
  const appId = identity.partnerAppId || this.gupshupAppId;
  const appApiKey = identity.appApiKey;
  const source = identity.source || this.whatsappPhoneNumber || this.bspDisplayPhoneNumber;

  // appId and source are mandatory — appApiKey can be dynamically resolved
  // via the partner app token service (getPartnerAppAccessToken)
  if (!appId || !source) {
    console.error(`[Workspace] BSP Ready check failed: appId=${!!appId}, apiKey=${!!appApiKey}, source=${!!source}`);
    throw Object.assign(new Error('Workspace Gupshup credentials missing. Ensure partnerAppId and source phone number are set.'), { code: 'CREDENTIALS_MISSING' });
  }

  if (!appApiKey) {
    console.warn(`[Workspace] appApiKey not stored locally for workspace ${this._id}. Will resolve dynamically via partner token service.`);
  }
