const PhoneThroughputLimiter = require('../../services/infrastructure/phoneThroughputLimiter');
...
async function checkPhoneThroughput(req, res, next) {
  try {
    if (!limiter) {
      limiter = new PhoneThroughputLimiter(null);
      logger.info('[PhoneThroughputMiddleware] Limiter initialized in fail-open mode (no Redis)');
    }
