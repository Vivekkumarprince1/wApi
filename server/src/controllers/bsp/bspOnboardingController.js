...
const { log: auditLog } = require('../../services/admin/auditService');
...
} = require('../../services/bsp/bspOnboardingServiceV2'); // Use V2 hardened service
...
async function startBspOnboarding(req, res) {
  try {
    ...
    logger.error('[BSP] Start onboarding error', { error: error.message, stack: error.stack });
    if (error.response) {
      logger.error('[BSP] Response status', { status: error.response.status });
      logger.error('[BSP] Response data', { data: error.response.data });
    }
...
async function registerPhoneForAppEndpoint(req, res) {
  try {
    ...
    logger.error('[BSP] Register phone error', { error: error.message, stack: error.stack });
...
async function handleCallback(req, res) {
  ...
  if (error) {
    logger.error('[BSP] Callback error from Gupshup', {
      error,
...
async function completeOnboarding(req, res) {
  try {
    ...
    logger.error('[BSP] Complete onboarding error', { error: error.message, stack: error.stack });
...
async function getStatus(req, res) {
  try {
    ...
    logger.error('[BSP] Get status error', { error: error.message, stack: error.stack });
    res.status(500).json({
      success: false,
...
async function disconnect(req, res) {
  try {
    ...
    logger.error('[BSP] Disconnect error', { error: error.message, stack: error.stack });
    res.status(500).json({
      success: false,
...
async function getStage1StatusEndpoint(req, res) {
  try {
    ...
    logger.error('[BSP] Get Stage 1 status error', { error: error.message, stack: error.stack });
    res.status(500).json({
      success: false,
...
async function triggerSync(req, res) {
  try {
    ...
    const { triggerWorkspaceSync } = require('../../services/bsp/gupshupAppSyncService');
    ...
    logger.error('[BSP] Manual sync error', { error: error.message, stack: error.stack });
    res.status(500).json({
      success: false,
