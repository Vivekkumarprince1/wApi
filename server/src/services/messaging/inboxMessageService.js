...
const slaService = require('../infrastructure/slaService');
const softLockService = require('../infrastructure/softLockService');
...
async function isWithin24HourWindow(conversationId) {
  const conversation = await Conversation.findById(conversationId)
    .select('lastCustomerMessageAt lastInboundAt windowExpiresAt isOpen status')
    .lean();

  if (!conversation) return false;

  const now = new Date();
  const isOpenByFlag = conversation.isOpen !== false;
  const isOpenByStatus = conversation.status !== 'closed';

  // New BSP specific check using windowExpiresAt
  if (conversation.windowExpiresAt) {
    return isOpenByFlag && isOpenByStatus && new Date(conversation.windowExpiresAt) > now;
  }

  // Legacy fallback check using lastCustomerMessageAt/lastInboundAt
  const lastInboundReference = conversation.lastCustomerMessageAt || conversation.lastInboundAt;
  if (lastInboundReference) {
    const timeSinceLastInbound = Date.now() - new Date(lastInboundReference).getTime();
    return isOpenByFlag && isOpenByStatus && timeSinceLastInbound < WINDOW_24H;
  }
...
async function sendTextMessage(options) {
  ...
  const rateLimitCheck = await agentRateLimitService.checkAgentRateLimit(workspaceId, agentId);
