...
export class InboxService {
  ...
  static async sendTextMessage(options: {
    ...
    const { workspaceId, conversationId, agentId, text, socketId } = options;
  ...
  /**
   * Send SMS message from inbox
   */
  static async sendSmsMessage(options: {
    workspaceId: string | Types.ObjectId;
    conversationId: string | Types.ObjectId;
    agentId: string | Types.ObjectId;
    text: string;
    ...
    return SmsService.sendSms({
      ...
      contactId,
      conversationId,
      sentBy: agentId as string
    });
  }

  /**
   * Send Email message from inbox
   */
  static async sendEmailMessage(options: {
    ...
    const { workspaceId, conversationId, agentId, subject, html } = options;
    const conversation = await Conversation.findById(conversationId).populate('contact');
    if (!conversation || !conversation.contact) throw new Error('CONVERSATION_NOT_FOUND');
    
    // @ts-ignore
    const email = conversation.contact.metadata?.email || conversation.contact.email;
    if (!email) throw new Error('CONTACT_EMAIL_NOT_FOUND');
  ...
  static async sendTemplateMessage(options: {
    ...
    const { workspaceId, conversationId, agentId, templateName, languageCode = 'en', variables = [], socketId } = options;
  ...
  static async sendMediaMessage(options: {
    ...
    const { workspaceId, conversationId, agentId, type, mediaUrl, mimeType, caption, filename, socketId } = options;
