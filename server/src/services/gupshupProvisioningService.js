...
async function provisionPartnerApp(userId, options = {}) {
    ...
    // STEP 5: Set Webhook Subscriptions (CRITICAL)
    console.log(`[Provisioning] Step 5: Setting Webhook Subscriptions for App ${appId}`);
    const modes = ['MESSAGE', 'FLOW_MESSAGE', 'BILLING', 'FAILED']; // We use V3 modes usually map to V3

    for (const mode of modes) {
        try {
            await gupshupService.setupWebhookSubscription({
                appId,
                appToken,
                mode,
                webhookUrl: `${process.env.API_BASE_URL || 'https://api.yourdomain.com'}/api/v1/webhook/gupshup`
            });
            console.log(`[Provisioning] Subscribed to ${mode}`);
        } catch (err) {
            console.warn(`[Provisioning] Failed to subscribe to ${mode} (Might already exist):`, err.message);
        }
    }
