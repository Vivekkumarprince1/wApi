...
const { Contact, Conversation, Message, Template, Deal, Pipeline, User } = require('../../models');
...
function getWhatsAppService() {
  return require('../bsp/bspMessagingService');
}
...
const actionHandlers = {
  ...
  async send_text_message(config, context) {
    ...
    // Send via WhatsApp service
    const whatsappService = getWhatsAppService();
    const result = await whatsappService.sendTextMessage(workspaceId, phone, messageContent);
  ...
  async send_media_message(config, context) {
    ...
    // Send via WhatsApp service
    const whatsappService = getWhatsAppService();
    const result = await whatsappService.sendMediaMessage(workspaceId, phone, mediaType || 'image', mediaUrl, messageContent);
