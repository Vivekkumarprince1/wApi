...
import { connectRedis, deleteKey, getJson, setJson } from '@/redis';
import { GupshupApp, Workspace } from '@/models';
import { decryptSecretCBC, encryptSecretCBC } from '@/services/security/secret-box';
...
async function readJson<T>(key: string): Promise<T | null> {
  ...
  return (await getJson(key)) as T | null;
}
