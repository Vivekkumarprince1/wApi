...
async function sendTemplate(params) {
  ...
  let response;
  try {
    // USE CENTRALIZED BSP MESSAGING SERVICE
    // This ensures: 1. Opt-out check, 2. Prerequisites (subscriptions), 3. Health check, 4. DLR timeout
    ...
    // Normalize response to match expected format if needed (bspMessagingService already does similar)
    if (!response.messages) {
      response.messages = [{ id: response.messageId || response.id }];
    }
  } catch (apiError) {
    // Only throw here, as bspMessagingService handles its own internal logging of failures
  ...
  const messageLog = await logTemplateSend({
    ...
    contactId,
    status: 'queued', // Changed from 'sent' to 'queued'
...
async function logTemplateSend(params) {
  ...
  const message = new Message({
    ...
    body: (function () {
      ...
      if (variables && variables.body && Array.isArray(variables.body)) {
        variables.body.forEach((val, i) => {
    ...
    template: {
      ...
      header: template.header?.enabled ? {
        ...
        mediaUrl: variables?.headerMedia || template.header.mediaUrl,
        mediaId: template.header.mediaHandle
      } : undefined,
      buttons: template.buttons?.enabled ? template.buttons.items.map(b => ({
        type: b.type,
        text: b.text,
        url: b.url,
