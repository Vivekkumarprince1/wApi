      ...
      <div className="max-w-[1400px] mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <p className="text-2xl font-bold text-slate-900 dark:text-slate-100">{workflows.filter(w => w.enabled).length}</p>
          </div>
          <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
            <p className="text-[10px] font-bold text-blue-400 uppercase tracking-widest mb-1">Total Executions</p>
            <p className="text-2xl font-bold text-slate-900 dark:text-slate-100">{workflows.reduce((acc, w) => acc + (w.executionCount || 0), 0)}</p>
          </div>
          <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
        ...
        {/* Filters & Search */}
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 mb-8 shadow-sm flex flex-col md:flex-row items-center gap-4">
          <div className="relative flex-1 w-full">
            ...
            <input 
              ...
              className="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl pl-11 pr-4 py-3 text-sm focus:ring-2 ring-primary/20 outline-none transition-all dark:text-slate-100 dark:placeholder:text-slate-500"
            />
          </div>
          <div className="flex items-center gap-3 w-full md:w-auto">
            <select 
              ...
              className="bg-slate-50 dark:bg-slate-800 border-none rounded-2xl px-4 py-3 text-sm outline-none cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors dark:text-slate-100"
            >
          ...
          {workflows.length === 0 ? (
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-slate-800 py-24 text-center">
              <div className="w-20 h-20 bg-slate-50 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-6">
                <Workflow className="h-10 w-10 text-slate-300 dark:text-slate-600" />
              </div>
              <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-2">No flows designed yet</h3>
              <p className="text-slate-500 dark:text-slate-400 mb-8 max-w-sm mx-auto">Create your first automated conversation flow to start saving time and engaging leads.</p>
              <button 
                onClick={() => router.push('/automation/workflows/create')}
              ...
              >
                <div className="flex items-center gap-6">
                  ...
                  <div className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center text-xl shadow-inner ${
                    wf.enabled ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600' : 'bg-slate-50 dark:bg-slate-800 text-slate-400'
                  }`}>
                  ...
                  <div className="flex-1 py-4">
                    <div className="flex items-center gap-3 mb-1">
                      <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 group-hover:text-primary transition-colors">{wf.name}</h3>
                      <span className={`px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase tracking-wider ${
                        wf.enabled ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'
                      }`}>
                        {wf.enabled ? 'Live' : 'Paused'}
                      </span>
                      {wf.flowConfig && (
                        <span className="px-2 py-0.5 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 rounded-lg text-[9px] font-extrabold uppercase tracking-tighter border border-indigo-100 dark:border-indigo-900/30 flex items-center gap-1">
                          <FaRobot size={10} /> Visual
                        </span>
                      )}
                    </div>
                    <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-1">{wf.description || 'Automating your business logic...'}</p>
                  </div>
                  ...
                  <div className="hidden lg:flex items-center gap-12 px-8 border-x border-slate-50 dark:border-slate-800">
                    <div>
                      ...
                      <p className="text-sm font-bold text-slate-900 dark:text-slate-100">{wf.executionCount || 0}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-xs font-medium text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md">
                  ...
                  <div className="flex items-center gap-2">
                    <button 
                      ...
                      className={`p-3 rounded-2xl transition-colors ${
                        wf.enabled ? 'bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-900/30' : 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-900/30'
                      }`}
                    >
                      {wf.enabled ? <ToggleLeft size={20} /> : <ToggleRight size={20} />}
                    </button>
                    <Link 
                      href={`/automation/workflows/edit/${wf._id}`}
                      className="p-3 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/30 rounded-2xl transition-colors"
                      title="View Flow"
                    >
                      <Eye size={20} />
                    </Link>
                    <Link 
                      href={`/automation/workflows/edit/${wf._id}`}
                      className="p-3 bg-violet-50 dark:bg-violet-900/20 text-violet-600 dark:text-violet-400 hover:bg-violet-100 dark:hover:bg-violet-900/30 rounded-2xl transition-colors"
                      title="Edit Flow"
                    >
                      <Pencil size={20} />
                    </Link>
                    <button 
                      ...
                      className="p-3 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/30 rounded-2xl transition-colors"
                      title="Delete"
                    >
                      <Trash2 size={20} />
                    </button>
