    ...
    const loadData = async () => {
      try {
        ...
        if (tplRes.status === 'fulfilled') {
          const approvedTemplates = tplRes.value.data || tplRes.value || [];
          setTemplates(approvedTemplates);
          if (approvedTemplates.length > 0) {
            setSelectedTemplate(approvedTemplates[0].name);
          }
        } else {
          console.error("Error loading templates:", tplRes.reason);
        }

        if (keysRes.status === 'fulfilled') {
          const keys = keysRes.value.data || keysRes.value || [];
          setApiKeys(keys);
          if (keys.length > 0) {
            setSelectedKey(keys[0].key);
          }
        } else {
          console.error("Error loading API keys:", keysRes.reason);
        }
      } catch (error) {
        console.error("Critical error in snippet generator:", error);
      } finally {
        setLoading(false);
      }
    };
