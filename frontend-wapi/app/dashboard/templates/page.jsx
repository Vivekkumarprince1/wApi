  ...
  const loadData = async (isBackground = false) => {
    try {
      ...
      if (templatesData?.templates) {
        templatesData.templates.forEach(t => templateMap.set(t._id || t.id, t));
      }
      
      if (deletedTemplatesData?.templates) {
        deletedTemplatesData.templates.forEach(t => templateMap.set(t._id || t.id, t));
      }
      ...
      if (categoriesData && categoriesData.categories) {
        const allCategories = [
          ...
          ...categoriesData.categories.map(cat => {
            const id = (cat.category || cat.name || cat._id || cat.id || 'UNKNOWN').toString();
            return {
              id,
                      ...
                      <td className="px-6 py-4 align-middle">
                        {template.qualityScore?.score ? (
                          <QualityScoreBadge 
                            ...
                            confidence={template.qualityScore.confidence}
                          />
                        ) : (
