...
const EngagementHeatmap = ({ matrix = [], maxEngagement = 0, isLoading = false }) => {
  ...
  if (isLoading) {
    return (
      <div className="w-full h-64 bg-gray-50 dark:bg-gray-900/20 animate-pulse rounded-xl flex items-center justify-center">
        <p className="text-gray-400">Loading Behavioral Map...</p>
      </div>
    );
  }

  return (
    <div className="p-6 bg-white dark:bg-gray-900/40 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-sm">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Engagement Heatmap</h3>
          <p className="text-sm text-gray-500">Intensity of message reads across the week</p>
        </div>
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[700px]">
          ...
          <div className="flex ml-10 mb-2">
            {hours.map((hour) => (
              <div key={hour} className="flex-1 text-[10px] text-gray-400 text-center">
                {hour % 3 === 0 ? `${hour}:00` : ''}
              </div>
            ))}
          </div>
            ...
            <div key={day} className="flex items-center mb-1 group">
              ...
              <div className="flex-1 flex gap-1">
                {matrix[dayIdx]?.map((count, hourIdx) => (
                  ...
                  >
                    {/* Tooltip */}
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-gray-900 text-white text-[10px] rounded opacity-0 group-hover/cell:opacity-100 pointer-events-none whitespace-nowrap z-20">
