...
if (!JWT_SECRET) {
  console.error('FATAL: JWT_SECRET environment variable is required for automation-service.');
  process.exit(1);
}
if (!INTERNAL_SECRET) {
  console.error('FATAL: INTERNAL_SERVICE_SECRET environment variable is required for automation-service.');
  process.exit(1);
}
