...
- **[MODIFY] src/app/api/auth/accept-invite/route.ts**: After activating the user, auto-login.
- **[MODIFY] src/app/auth/accept-invite/page.tsx**: Redirect directly to `/dashboard`.
- **[MODIFY] src/middleware.ts**: Allow `/onboarding/*` routes without redirecting to login when token exists.
- **[MODIFY] src/app/api/auth/session/route.ts**: Remove development bypasses and include BSP status.
...
- **[NEW] src/app/api/onboarding/bsp/start/route.ts**: Provisions Gupshup app.
- **[NEW] src/app/api/onboarding/bsp/callback/route.ts**: Callback handler.
- **[NEW] src/app/api/onboarding/bsp/complete/route.ts**: Completes onboarding.
