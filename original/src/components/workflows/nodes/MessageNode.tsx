...
export default function MessageNode({ data, selected }: { data: any, selected: boolean }) {
  const actionType = String(data?.type || '').toLowerCase();

  const getPreview = () => {
    if (actionType === 'add_tag') {
      return data.tag ? `Add tag: ${data.tag}` : 'Configure tag to add...';
    }

    if (actionType === 'assign_conversation') {
      return data.assignedTo ? `Assign to: ${data.assignedTo}` : 'Configure assignee...';
    }

    if (actionType === 'create_deal') {
      const title = data.title || 'Untitled deal';
      const value = data.value ? ` (Value: ${data.value})` : '';
      return `Create deal: ${title}${value}`;
    }

    return data.messageContent ? `"${data.messageContent}"` : 'Configure message payload...';
  };
