  ...
  const sendMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      if (data.conversationId) {
        router.push(`/dashboard/inbox?conversationId=${data.conversationId}`);
      }
      */
    },
    onError: (error: any) => {
      if (error?.status === 402) {
        ...
        setTimeout(() => router.push('/dashboard/billing?action=recharge'), 1500);
      } else {
                 ...
                 <div className="pt-6 flex flex-col gap-4">
                    {isInsufficientBalance ? (
                       <div className="flex flex-col gap-4">
                          ...
                          <Button 
                            onClick={() => router.push('/dashboard/billing?action=recharge')}
