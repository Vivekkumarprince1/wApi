...
export function DashboardHeader() {
  ...
  const settingsItems = [
    { label: 'WhatsApp Profile', path: '/dashboard/settings/whatsapp-profile', icon: UserIcon, color: 'bg-violet-500/10 text-violet-600 dark:text-violet-400' },
    ...
    { label: 'Member Profile', path: '/dashboard/settings/member-profile', icon: UserIcon, color: 'bg-amber-500/10 text-amber-600 dark:text-amber-400' },
    { label: 'Manage Events', path: '/dashboard/settings/developer/webhooks', icon: Calendar, color: 'bg-indigo-500/10 text-indigo-600 dark:text-indigo-400' },
    ...
    <header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b border-border/50 bg-background/80 backdrop-blur-md px-4 lg:px-6">
      <div className="flex items-center gap-4">
        ...
        <Breadcrumb className="hidden md:flex">
          <BreadcrumbList>
            {/* <BreadcrumbItem>
              <BreadcrumbLink href="/dashboard" className="text-muted-foreground/60 hover:text-primary transition-colors">wApi</BreadcrumbLink>
            </BreadcrumbItem> */}
        ...
        >
          ...
          <div className="flex flex-col gap-0">
            ...
            <span className="text-sm font-black text-foreground">
              {wallet?.currency || '₹'} {wallet?.balance?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || '0.00'}
            </span>
          </div>
          ...
          <DropdownMenuContent className="w-64 rounded-2xl shadow-premium border-border/50 p-2" align="end">
            ...
            <DropdownMenuGroup className="p-1 space-y-1">
              <DropdownMenuItem
                ...
                onClick={() => router.push('/dashboard/settings/member-profile')}
              >
