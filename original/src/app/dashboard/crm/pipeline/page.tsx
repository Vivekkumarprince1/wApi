...
export default function PipelinePage() {
  ...
  const moveMutation = useMutation({
    ...
    onMutate: async (newMove) => {
      ...
      queryClient.setQueryData(['deals', activePipeline?._id], (old: Deal[] | undefined) => {
        if (!old) return [];
        return old.map(d => d._id === newMove.id ? { ...d, stage: newMove.stage } : d);
      });
          ...
          {isDealsLoading ? (
            ...
            <AnimatePresence mode="wait">
              {viewMode === 'board' ? (
                ...
                >
                   <PipelineAutomation pipelineId={activePipeline._id} stages={activePipeline.stages} />
                </motion.div>
              ) : (
