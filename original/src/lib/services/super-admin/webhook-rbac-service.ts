...
import type { UserRole } from '@/lib/models/auth/User';
import {
  WebhookPolicy,
...
} from '@/lib/models/super-admin/WebhookPolicy';
...
export function getWebhookModeCatalog() {
  return WEBHOOK_SUBSCRIPTION_MODES.map((mode) => ({ mode, ...WEBHOOK_MODE_CATALOG[mode] }));
}
