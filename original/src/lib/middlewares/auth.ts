...
import { getWorkspaceAccessDecision, shouldBypassWorkspaceAccessGuard } from "@/lib/services/workspace-access-service";
...
export function withFeature(featureSlug: string, handler: AuthenticatedHandler) {
  return withAuth(async (req, context) => {
    ...
    const features = workspace?.plan?.features || workspace?.features || [];
