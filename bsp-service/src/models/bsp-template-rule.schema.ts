...
export type BspTemplateRuleDocument = HydratedDocument<BspTemplateRule>;
...
export class BspTemplateRule extends WorkspaceScopedModel {
  @Prop({ required: true })
  name!: string;
  ...
  // Template id (BspTemplateMirror) or template name to send when the rule fires.
  @Prop()
  template?: string;
...
export const BspTemplateRuleSchema = SchemaFactory.createForClass(BspTemplateRule);
BspTemplateRuleSchema.index({ workspaceId: 1, enabled: 1, priority: -1 });
