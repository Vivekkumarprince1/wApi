...
export class BspEsbFlow extends Document {
...
export const BspEsbFlowSchema = SchemaFactory.createForClass(BspEsbFlow);

// Indexes
BspEsbFlowSchema.index({ workspace: 1, provider: 1 });
BspEsbFlowSchema.index({ status: 1, updatedAt: -1 });
BspEsbFlowSchema.index({ callbackState: 1 }, { sparse: true });
