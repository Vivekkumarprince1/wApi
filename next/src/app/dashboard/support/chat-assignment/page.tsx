  ...
  const fetchData = async () => {
    ...
    try {
      const [settingsRes, agentsRes, rulesRes] = await Promise.all([
        fetch('/api/inbox/settings').then(r => r.json()),
        fetch('/api/workspace/team/members').then(r => r.json()),
        fetch('/api/automation/engine/rules?trigger=conversation.created').then(r => r.json())
      ]);

      if (settingsRes.success) setInboxSettings(settingsRes.data);
      if (agentsRes.success) {
        const agentArray = agentsRes.data.members || (Array.isArray(agentsRes.data) ? agentsRes.data : []);
                               ...
                               <div className="flex items-center gap-3">
                                  <Switch 
                                    ...
                                    onCheckedChange={async (val) => {
                                      try {
                                        const res = await fetch(`/api/automation/engine/rules/${rule._id}/toggle`, {
                                          method: 'POST',
                                          headers: { 'Content-Type': 'application/json' },
