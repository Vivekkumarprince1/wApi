...
export default function MacrosPage() {
  ...
  const { data: macros, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axios.get('/api/messaging/quick-replies');
  ...
  const mutation = useMutation({
    mutationFn: async (data: any) => {
      if (editingMacro) {
        return axios.put(`/api/messaging/quick-replies/${editingMacro._id}`, data);
      }
      return axios.post('/api/messaging/quick-replies', data);
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => axios.delete(`/api/messaging/quick-replies/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['quick-replies'] });
        ...
        <div className="md:col-span-4 bg-muted/30 border border-border/50 rounded-2xl p-4 flex flex-col md:flex-row items-center gap-4">
          ...
          <div className="flex items-center gap-6 px-4 py-2 bg-background/50 rounded-xl border border-border/40">
            ...
            <div className="text-center">
              ...
              <p className="text-sm font-black text-emerald-500">{macros?.filter((m: any) => m.isActive).length || 0}</p>
            </div>
          </div>
        </div>
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-black text-primary uppercase tracking-widest">/{macro.shortcut.replace('/', '')}</p>
                    </div>
                  </div>
