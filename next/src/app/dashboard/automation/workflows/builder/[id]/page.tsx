...
export default function WorkflowBuilderPage() {
  ...
  const saveWorkflow = async () => {
    try {
      ...
      if (isCreateMode && data?.data?._id) {
        router.push(`/dashboard/automation/workflows/builder/${data.data._id}`);
      } else {
        router.push('/dashboard/automation/workflows');
      }
    } catch (error: any) {
    ...
    <div className="flex flex-col h-screen w-full bg-slate-50">
      <div className="h-16 bg-white border-b border-slate-200 px-4 flex items-center justify-between shrink-0 z-10 shadow-sm">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => router.push('/dashboard/automation/workflows')}>
