...
export default function AutomationHubPage() {
  const { data, isLoading } = useQuery({
    queryKey: ['automation-hub-all-modules'],
    queryFn: async () => {
      const [
        rulesRes,
        intentsRes,
        answerBotSettingsRes,
        answerBotSourcesRes,
        answerBotFaqsRes,
        formsRes,
        interaktiveRes,
        ...
        getAnswerBotSettings(),
        getAnswerBotSources(),
        getAnswerBotFAQs(undefined, { status: 'draft', limit: 200 }),
        fetchWhatsAppForms(),
        fetchInteraktiveLists(),
        fetchInstagramQuickflows(),
        getAutomationStats({ days: 7 }),
      ]);

      const getValue = (result: PromiseSettledResult<any>) =>
        result.status === 'fulfilled' ? result.value : null;

      const rulesPayload = getValue(rulesRes);
      const intentsPayload = getValue(intentsRes);
      const answerBotSettingsPayload = getValue(answerBotSettingsRes);
      const answerBotSourcesPayload = getValue(answerBotSourcesRes);
      const answerBotFaqsPayload = getValue(answerBotFaqsRes);
      const formsPayload = getValue(formsRes);
      const interaktivePayload = getValue(interaktiveRes);
      const quickflowsPayload = getValue(quickflowsRes);
      const automationStatsPayload = getValue(automationStatsRes);

      const rules = safeArray(rulesPayload?.data?.rules);
      const workflows = rules.filter((rule: any) => rule.category === 'workflow');
      const autoReplies = rules.filter((rule: any) => rule.category === 'auto_reply');
      const activeRules = rules.filter((rule: any) => rule.enabled);

      const aiIntentRules = safeArray(intentsPayload?.data?.rules);
      const forms = safeArray(formsPayload?.data);
      const interaktiveLists = safeArray(interaktivePayload?.data);
      const quickflows = safeArray(quickflowsPayload);
      const answerBotSources = safeArray(answerBotSourcesPayload?.sources);
      const answerBotDraftFaqs = safeArray(answerBotFaqsPayload?.faqs);

      const answerBotEnabled = Boolean(answerBotSettingsPayload?.settings?.enabled);

      const enabledInteraktive = interaktiveLists.filter((list: any) => list.enabled).length;
      const enabledQuickflows = quickflows.filter((flow: any) => flow.enabled).length;
      const publishedForms = forms.filter((form: any) => form.status === 'published').length;

      const executionOverview = automationStatsPayload?.data?.overview || {
        total: 0,
        success: 0,
        failed: 0,
        skipped: 0,
      };
      const successRate = automationStatsPayload?.data?.successRate || 0;

      return {
        rules,
        workflowsCount: workflows.length,
        autoRepliesCount: autoReplies.length,
        activeRulesCount: activeRules.length,
        aiIntentsCount: aiIntentRules.length,
        answerBotEnabled,
        answerBotSourcesCount: answerBotSources.length,
        answerBotDraftFaqsCount: answerBotDraftFaqs.length,
        formsCount: forms.length,
        publishedFormsCount: publishedForms,
        interaktiveListsCount: interaktiveLists.length,
        enabledInteraktiveCount: enabledInteraktive,
        quickflowsCount: quickflows.length,
        enabledQuickflowsCount: enabledQuickflows,
        executionOverview,
        successRate,
      };
    },
  ...
  const moduleCards: ModuleCard[] = [
    ...
    },
    {
      ...
      href: '/dashboard/automation/answerbot',
    ...
    },
    {
      ...
      href: '/dashboard/automation/ai-intent-matching',
    ...
    },
    {
      ...
      href: '/dashboard/automation/whatsapp-forms',
    ...
    },
    {
      ...
      href: '/dashboard/automation/interaktive-list',
      countLabel: 'Enabled',
    ...
    },
    {
      ...
      href: '/dashboard/automation/instagram-quickflows',
      countLabel: 'Enabled',
      ...
      <div className="rounded-3xl border border-border/60 bg-gradient-to-br from-primary/10 via-background to-background p-5 lg:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <Link href="/dashboard/automation/workflows/builder/create">
              <Button className="rounded-xl h-10 px-4 font-bold">
                <Plus className="h-4 w-4 mr-2" />
                New Workflow
              </Button>
            </Link>
            <Link href="/dashboard/automation/auto-replies">
