...
import { Workspace, User, Message, WalletTransaction } from "@/lib/models";
...
export const GET = withRole(['super_admin'], async (req: NextRequest) => {
  try {
    ...
    // 4. Active Revenue (All completed RECHARGE transactions)
    const revenueResult = await WalletTransaction.aggregate([
      { $match: { type: 'RECHARGE', status: 'COMPLETED' } },
      { $group: { _id: null, total: { $sum: '$amount' } } }
    ]);
    const activeRevenue = revenueResult.length > 0 ? revenueResult[0].total : 0;
