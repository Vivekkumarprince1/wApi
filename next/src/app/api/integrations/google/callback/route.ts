...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}/dashboard/integrations?success=gs_connected`);
  } catch (err: any) {
    console.error('[GoogleCallback] Error:', err.message);
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}/dashboard/integrations?error=gs_failed`);
  }
}
