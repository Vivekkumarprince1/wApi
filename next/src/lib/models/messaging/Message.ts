...
export type MessageType = 'text' | 'template' | 'image' | 'video' | 'document' | 'audio' | 'sticker' | 'location' | 'contacts' | 'interactive' | 'reaction' | 'note' | 'payment' | 'pix' | 'boleto';
...
const MessageSchema = new Schema<IMessageDocument, IMessageModel>({
  ...
  type: { 
    type: String, 
    enum: ['text', 'template', 'image', 'video', 'document', 'audio', 'sticker', 'location', 'contacts', 'interactive', 'reaction', 'note', 'payment', 'pix', 'boleto'], 
    default: 'text', 
    index: true 
  },
  body: { type: String },
