...
export class WabaService {
  ...
  static async sendTemplateMessage(
    ...
    const cost = await PricingService.getCost(workspaceId, category);
    ...
    try {
      ...
      if (cost > 0 && !options.campaignId) {
          const workspace = await Workspace.findById(workspaceId).select('walletBalance').lean();
          ...
          await LedgerService.deduct(workspaceId, cost, {
            type: 'SPEND',
            referenceType: 'AUTOMATION_OR_WAP_DIRECT',
      ...
      if (deducted && cost > 0) {
        try {
          await LedgerService.credit(workspaceId, cost, {
            type: 'REFUND',
            referenceType: 'EXCEPTION_DURING_DISPATCH',
  ...
  static async syncIdentityFromProvider(workspaceId: string | Types.ObjectId, appId?: string) {
    ...
    try {
      ...
      if (redis) {
         await redis.set(syncFailedKey, 'true', { EX: 900 }); 
      }
