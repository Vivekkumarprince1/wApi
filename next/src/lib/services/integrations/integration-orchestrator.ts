...
export class IntegrationService {
  ...
  static async handleExternalEvent(
    ...
    // 3. Trigger Automation Service
    await AutomationService.trigger(workspaceId, normalizedData.eventType, {
      ...normalizedData.data,
      contactId: contactId?.toString(),
      provider
    });
    
    console.log(`[IntegrationService] ✅ Processed ${provider} event: ${normalizedData.eventType} for Workspace ${workspaceId}`);
  }
  ...
  private static normalize(provider: IntegrationProvider, payload: any) {
    switch (provider) {
      ...
      case 'meta-ads':
        // Meta Lead Ads payload usually comes as lead_id, needs enrichment from Meta Graph API
        return {
          eventType: 'NEW_LEAD',
