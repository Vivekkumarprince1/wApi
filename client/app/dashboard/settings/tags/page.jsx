    ...
    <div className=" p-6">
      <div className="max-w-5xl mx-auto mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Categorize contacts with tags</p>
        </div>
      </div>

      <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-6">
        <form onSubmit={createTag} className="bg-card rounded-xl shadow-premium p-6">
          <h2 className="text-lg font-semibold text-foreground mb-4">Create Tag</h2>
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm text-foreground mb-1">Color</label>
              <input type="color" value={form.color} onChange={(e)=>setForm({...form,color:e.target.value})} className="w-16 h-10 p-1 border border-border rounded" />
            </div>
            <button type="submit" className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-amber-500 text-white rounded-xl hover:bg-amber-600">
              <FaPlus/> Create Tag
            </button>
          </div>
        </form>
        ...
        <div className="bg-card rounded-xl shadow-premium p-6">
          <h2 className="text-lg font-semibold text-foreground mb-4">Tags ({tags.length})</h2>
          {tags.length === 0 ? (
              ...
              <li key={t._id || t.id} className="border border-border rounded p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{t.contactCount || t.count || 0} contacts</p>
                  </div>
                </div>
                <button onClick={()=>deleteTag(t._id || t.id)} className="text-destructive hover:text-destructive/80"><FaTrash/></button>
              </li>
            ))}
          </ul>
          )}
