...
import { getAutoReply, updateAutoReply, getTemplates, getTemplate } from '@/lib/api';
...
export default function EditAutoReplyPage() {
  ...
  const loadAutoReply = async () => {
    try {
      const data = await getAutoReply(params.id);
      setFormData({
        keywords: data.keywords || [''],
        template: data.template?._id || data.template || '',
        matchMode: data.matchMode || 'contains',
        triggerType: data.triggerType || 'keyword',
        enabled: data.enabled !== false
      });
    } catch (err) {
      setError('Failed to load auto-reply: ' + err.message);
      console.error(err);
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateAutoReply(params.id, {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-muted-foreground">Loading auto-reply...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-card border-b border-border">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <h1 className="text-3xl font-bold text-foreground">Edit Auto-Reply</h1>
          <p className="text-muted-foreground mt-1">Update auto-reply settings</p>
        </div>
      </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <>
                  <div className="bg-card rounded-xl border border-border shadow-sm p-6 animate-fade-in">
                    ...
                    <p className="text-sm text-muted-foreground mb-4">
                      Match any of these keywords to trigger the reply.
                    </p>
              ...
              <div className="bg-card rounded-xl border border-border shadow-sm p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-lg font-semibold text-foreground">Enable Auto-Reply</h2>
                    <p className="text-sm text-muted-foreground">Turn this rule on or off</p>
                  </div>
