...
export default function EditWorkflowPage() {
  ...
  const loadWorkflow = async () => {
    try {
      const workflow = await get(`/automation/${workflowId}`);
      setForm(workflow);
      
      // Set condition type if exists
      if (workflow.condition?.type === 'keyword') {
        setConditionType('keyword');
        setConditionKeywords(workflow.condition.keywords?.join(', ') || '');
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await put(`/automation/${workflowId}`, form);
      router.push(`/automation/workflows/view/${workflowId}`);
    } catch (err) {
