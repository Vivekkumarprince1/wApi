...
export default function DealCard({ deal, stage, onDelete, onOpenDetail }) {
  ...
  return (
    <div
      ...
      className="bg-white dark:bg-muted border border-gray-200 dark:border-border rounded-lg p-3 mb-2 cursor-move hover:shadow-md dark:hover:shadow-lg transition-shadow group"
    >
      {/* Header: Contact Name */}
      <div className="flex items-start justify-between mb-2">
        ...
        {isHovering && (
          <button
            ...
            className="ml-2 text-destructive hover:text-destructive/80 dark:hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity"
            title="Delete deal"
          >
            <Trash2 size={14} />
          </button>
      ...
      {deal.notes && deal.notes.length > 0 && (
        <div className="flex items-center gap-1 text-xs text-muted-foreground mt-2 pt-2 border-t border-gray-200 dark:border-border">
          <FileText size={12} />
