...
const SignupOTP = ({ 
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // Send only the fields expected by the backend: email, otp, name, password
  ...
  const handleResendOTP = async () => {
    setLoading(true);
    try {
      // Send only the fields expected by the backend for OTP resend
      await sendSignupOTP({
        email,
        name: tempData.name,
        password: tempData.password
      ...
      setCanResend(false);
      setOtp(['', '', '', '', '', '']);
      onError('OTP resent successfully!');
    } catch (error) {
      onError(error.message);
    } finally {
      setLoading(false);
