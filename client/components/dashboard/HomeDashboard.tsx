    ...
    const loadData = async () => {
      try {
        ...
        const [response, messageMetrics] = await Promise.all([
          get('/analytics/dashboard/overview').catch((err) => {
            console.error('Failed to fetch analytics:', err);
            return { data: null };
          }),
          get('/metrics/messages?days=7').catch((err) => {
            console.error('Failed to fetch message metrics:', err);
            return null;
          })
        ]);
            ...
            <div className="bg-card rounded-2xl p-5 border border-border/50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="rounded-xl border border-border/60 p-4 bg-background/60">
                  ...
                  <div className="space-y-2">
                    {Object.entries(deliveryHealth.byCountry || {}).slice(0, 4).map(([country, count]: any) => (
                ...
                <div className="rounded-xl border border-border/60 p-4 bg-background/60">
                  ...
                  {deliveryHealth.topFailures?.length > 0 ? (
                    <div className="space-y-2">
                      {deliveryHealth.topFailures.slice(0, 3).map((item: any) => (
