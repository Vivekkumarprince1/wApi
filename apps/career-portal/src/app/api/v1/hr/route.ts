...
import { canAccessAdminArea, createHrUser, isSuperAdminUser, listAuthUsers } from "@/lib/auth-store";
...
export function GET(request: NextRequest) {
  ...
  if (!isSuperAdminUser(user)) return forbidden("Only super admins can list HR users.");

  return NextResponse.json({ data: listAuthUsers().filter((item) => canAccessAdminArea(item)) });
}
