...
import { bulkIssueOffers } from "@/modules/documents/server/documents";
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(await bulkIssueOffers(await file.text(), actor));
  } catch (error) {
    return apiErrorResponse(error, "Unable to import offers");
  }
}
