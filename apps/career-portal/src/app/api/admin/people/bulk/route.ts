...
import { importEmployees } from "@/modules/people/server/people";
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(await importEmployees(await file.text(), actor));
  } catch (error) {
    return apiErrorResponse(error, "Unable to import employees");
  }
}
