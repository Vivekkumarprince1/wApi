...
import { Send, UserPlus, Star, MessageCircle, AlertTriangle, Zap, LucideIcon } from 'lucide-react';
...
const QuickAction = ({ title, description, icon: Icon, color, onClick, badge, canUseMessaging, router }: QuickActionProps) => {
  const isLockedAction = !canUseMessaging && ['Send Campaign', 'Create Template', 'View Inbox'].includes(title);
  
  return (
    <button
      onClick={isLockedAction ? () => router.push('/?connectWhatsApp=1') : onClick}
...
const QuickActions = ({ canUseMessaging, onAddContacts }: QuickActionsProps) => {
  ...
  return (
    <div className="bg-card rounded-2xl p-5 border border-border/50">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <QuickAction canUseMessaging={canUseMessaging} router={router} title="Send Campaign" description="Reach thousands" icon={Send} color="from-emerald-500 to-emerald-600" onClick={() => router.push('/campaign')} />
        ...
        <QuickAction canUseMessaging={canUseMessaging} router={router} title="Create Template" description="Design messages" icon={Star} color="from-violet-500 to-violet-600" onClick={() => router.push('/templates')} badge="New" />
        <QuickAction canUseMessaging={canUseMessaging} router={router} title="View Inbox" description="Check responses" icon={MessageCircle} color="from-amber-500 to-amber-600" onClick={() => router.push('/inbox')} />
      </div>
    </div>
  );
};
