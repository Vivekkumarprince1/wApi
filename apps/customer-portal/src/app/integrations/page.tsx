      ...
      <div className="flex items-center gap-3 overflow-x-auto pb-2 scrollbar-hide py-1">
        {CATEGORIES.map((cat) => (
          <Badge
            ...
            className={`h-10 px-6 rounded-2xl font-black text-[9px] uppercase tracking-[0.15em] border-none cursor-pointer whitespace-nowrap transition-all duration-300 active:scale-95 ${selectedCategory === cat
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredApps.map((app) => (
            <Card key={app.id} className={`group border-none ring-1 ring-border/50 bg-background/50 backdrop-blur-xl shadow-lg hover:shadow-2xl hover:ring-primary/30 transition-all duration-500 relative overflow-hidden rounded-[2.5rem] ${app.isComingSoon ? 'opacity-60 pointer-events-none' : ''
              }`}>
              {app.active && (
