...
export async function deleteWorkspace(workspaceId: string) {
  ...
  const ws = await Workspace.findById(workspaceId).lean<{ _id: unknown; owner?: unknown } | null>();
  if (!ws) throw new Error("Workspace not found");

  if (ws.owner) {
    const owner = await User.findById(String(ws.owner)).lean<{ role?: string } | null>();
...
export async function impersonateWorkspace(workspaceId: string, adminId: string) {
  const { Workspace, User } = await coreModels();
  const ws = await Workspace.findById(workspaceId).lean<{ _id: unknown; name?: string; owner?: unknown } | null>();
  if (!ws) throw new Error("Workspace not found");
  ...
  if (ws.owner) {
    owner = await User.findById(String(ws.owner)).select("email").lean<OwnerLean>();
  }
  if (!owner) {
