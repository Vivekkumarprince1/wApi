  ...
  async sendMessage(req: AuthRequest, res: Response, next: any) {
    try {
      ...
      if (isInternalNote) {
        ...
        // Emit socket event
        SocketService.emitMessageSent(workspace._id, conversation._id, newMessage, user);
      ...
      if (type === 'template' && template) {
        ...
        // Emit socket event
        SocketService.emitMessageSent(workspace._id, conversation._id, (result as any).message, user);
        SocketService.emitStatusUpdate(workspace._id, conversation._id, (result as any).message._id, 'sent', new Date());
