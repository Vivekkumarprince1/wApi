import { Business, User, Workspace } from '@/models';
  ...
  static async runProvisioningPipeline(workspaceId: string) {
    ...
    try {
      ...
      if (workspace.onboardingStatus === 'PROVISIONING_STARTED') {
        ...
        console.log(`[Orchestrator] Step 2 Success: App Assigned for ${workspaceId}`);
      ...
      if (ws.onboardingStatus === 'APP_ASSIGNED') {
        ...
        if (appId && !appId.startsWith('mock_')) {
          ...
          if (token) {
            ...
            console.log(`[Orchestrator] Token encrypted and stored for ${appId}`);
          } else {
            console.warn(`[Orchestrator] No token returned for ${appId}`);
          }
        } else {
          console.log(`[Orchestrator] Skipping token resolution for ${appId} (Mock App)`);
        }
        ws.onboardingStatus = 'TOKEN_RESOLVED';
        await ws.save();
        console.log(`[Orchestrator] Step 3 Success: Token Resolved for ${workspaceId}`);
      }
      ...
      if (wsAfterToken.onboardingStatus === 'TOKEN_RESOLVED') {
        ...
        if (appId && !appId.startsWith('mock_')) {
          ...
          if (wsAfterToken.esbFlow?.contactSyncFingerprint !== contactFingerprint) {
            console.log(`[Orchestrator] Syncing new contact info for ${appId}`);
            ...
            console.log(`[Orchestrator] Contact info unchanged, skipping sync for ${appId}`);
          }
        }
        wsAfterToken.onboardingStatus = 'CONTACTS_SET';
        await wsAfterToken.save();
        console.log(`[Orchestrator] Step 4 Success: Contacts Set for ${workspaceId}`);
      }
      ...
      if (wsAfterContacts.onboardingStatus === 'CONTACTS_SET') {
        ...
        if (appId && !appId.startsWith('mock_')) {
          ...
          if (webhookUrl) {
            console.log(`[Orchestrator] Subscribing ${appId} to ${webhookUrl}`);
            ...
            wsAfterContacts.esbFlow = {
             ...
             console.error(`[Orchestrator] Could not resolve webhook URL for ${appId}`);
        ...
        console.log(`[Orchestrator] Step 5 Success: Pipeline COMPLETED for ${workspaceId}`);
