...
function getVerificationMode() {
  return String(config.businessVerificationProvider || 'hybrid').toLowerCase();
}

function allowMockFallback() {
  const mode = getVerificationMode();
  return mode === 'mock' || (mode === 'hybrid' && config.env !== 'production' && !config.cleartaxApiKey && !config.karzaApiKey);
}
...
function isProductionVerificationMode() {
  return config.env === 'production' && getVerificationMode() !== 'mock';
}
...
async function verifyWithCleartax(input: BusinessVerificationInput): Promise<VerificationOutcome> {
  ...
  const response = await requestWithRetry(async () => {
    if (gstNumber) {
      return axios.get(`${config.cleartaxBaseUrl.replace(/\/$/, '')}/gst/v1/taxpayer/${gstNumber}`, {
        headers: {
          'x-api-key': config.cleartaxApiKey,
    ...
    return axios.get(`${config.cleartaxBaseUrl.replace(/\/$/, '')}/msme/v1/udyam/${msmeNumber}`, {
      headers: {
        'x-api-key': config.cleartaxApiKey,
        Accept: 'application/json'
      },
...
async function verifyKarzaDocument(documentType: 'pan' | 'msme', documentNumber: string, businessName?: string): Promise<VerificationOutcome> {
  if (!config.karzaApiKey) {
    throw Object.assign(new Error('Karza API key is not configured'), { status: 503, code: 'KARZA_NOT_CONFIGURED' });
  }

  const baseUrl = config.karzaBaseUrl.replace(/\/$/, '');
  const endpointPath = documentType === 'pan' ? config.karzaPanVerifyPath : config.karzaMsmeVerifyPath;
