...
if (process.env.NODE_ENV === 'production') {
  if (!_jwtSecret || _jwtSecret === 'your-secret-key-change-in-production') {
    throw new Error('FATAL: A secure, non-default JWT_SECRET environment variable is required in production.');
  }
  if (!_internalServiceSecret || _internalServiceSecret === 'dev-internal-service-secret-change-me') {
...
export const config = {
  ...
  jwtSecret: _jwtSecret || 'your-secret-key-change-in-production',
  ...
  internalServiceSecret: _internalServiceSecret || 'dev-internal-service-secret-change-me',
