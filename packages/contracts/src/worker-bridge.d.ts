 ...
 */
export type WorkerBridgeAction = 'send-template' | 'preflight-validate' | 'socket-broadcast' | 'get-pricing' | 'get-template' | 'get-contact' | 'query-contacts' | 'count-contacts' | 'billing-park' | 'billing-settle';
...
export type WorkerBridgeRequest = {
    ...
    data: PreflightValidateData;
} | {
    action: 'socket-broadcast';
    data: SocketBroadcastData;
} | {
    action: 'get-pricing';
    data: GetPricingData;
} | {
    ...
    data: QueryContactsData;
} | {
    action: 'count-contacts';
    data: QueryContactsData;
} | {
    action: 'billing-park';
    data: BillingParkData;
} | {
    action: 'billing-settle';
    data: BillingSettleData;
};
