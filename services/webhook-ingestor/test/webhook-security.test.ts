...
import { verifyProviderSignature } from '../src/webhook-security';

const body = Buffer.from('{"event":"message"}');
const secrets = { gupshup: 'gupshup-secret', meta: 'meta-secret', instagram: 'instagram-secret' };
...
test('rejects missing, invalid, and cross-provider signatures', () => {
    const metaSignature = `sha256=${crypto.createHmac('sha256', secrets.meta).update(body).digest('hex')}`;
    ...
    assert.equal(verifyProviderSignature({ provider: 'instagram', rawBody: body, headers: { 'x-hub-signature-256': metaSignature }, secrets }), false);
});