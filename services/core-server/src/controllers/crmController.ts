  ...
  async getDeals(req: AuthRequest, res: Response) {
    try {
      ...
      if (segmentId && !contactId) {
        try {
          const response = await proxyController.forwardToService('campaign', {
            method: 'GET',
            path: `/segments/${segmentId}/resolve`,
          ...
          if (contactIds.length > 0) {
            ...
            // If segment is empty, return no deals
            return res.json({ success: true, data: [], pagination: { total: 0, page, limit, pages: 0 } });
          }
        } catch (err: any) {
          console.error('[CRM:SegmentResolve] Failed:', err.message);
        }
      }
