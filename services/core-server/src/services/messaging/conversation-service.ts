...
export class ConversationService {
   ...
   */
  static async syncMessage(messageId: string | Types.ObjectId, socketId?: string) {
    ...
    if (message.direction === 'inbound') {
      ...
      const sentBy = message.sentBy ? await (require('@/models').User.findById(message.sentBy).select('name')) : null;
      await SocketService.emitMessageSent(workspaceIdStr, conversationIdStr, message, sentBy, socketId);
    }
    ...
    emitToWorkspace(workspaceIdStr, "inbox:conversation_updated", listPayload);
